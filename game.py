...
class CFRRL_Game(Game):
    ...
    #         if value == 1.0:
    #             print(f"solution: {key} -> {value}")
    #             solution_rs.append(key)
    #     print("MLU", mlu)
    #     print("delay", delay)
    ...
    def evaluate(self, tm_idx, actions=None, ecmp=True, eval_delay=False):
        ...
        line = []
        line.append(f"mlu chuẩn hóa: {norm_mlu:.3f}\n")
        line.append(f"mlu: {mlu:.3f}\n")
        line.append(f"mlu chuẩn hóa (crit): {norm_crit_mlu:.3f}\n")
        line.append(f"mlu (crit): {crit_mlu:.3f}\n")
        line.append(f"mlu chuẩn hóa (topk): {norm_topk_mlu:.3f}\n")
        line.append(f"mlu (topk): {topk_mlu:.3f}\n")

        if ecmp:
            norm_ecmp_mlu = optimal_mlu / ecmp_mlu
            line.append(f"mlu chuẩn hóa (ecmp): {norm_ecmp_mlu:.3f}\n")
            line.append(f"mlu (ecmp): {ecmp_mlu:.3f}\n")

        if eval_delay:
            solution = self.optimal_routing_delay(tm_idx)
            optimal_delay = self.eval_optimal_routing_delay(tm_idx, solution)

            line.append(f"delay chuẩn hóa: {optimal_delay/delay:.3f}\n")
            line.append(f"delay chuẩn hóa (crit): {optimal_delay/crit_delay:.3f}\n")
            line.append(f"delay chuẩn hóa (topk): {optimal_delay/topk_delay:.3f}\n")
            line.append(f"delay chuẩn hóa (optimal_mlu_delay): {optimal_delay/optimal_mlu_delay:.3f}\n")
            if ecmp:
                line.append(f"delay chuẩn hóa (ecmp): {optimal_delay/ecmp_delay:.3f}\n")

            assert tm_idx in self.load_multiplier, (tm_idx)
            line.append(f"load_multiplier: {self.load_multiplier[tm_idx]:.3f}\n")

        # In các giá trị theo yêu cầu
        result_line = f"idx: {tm_idx}, " + ", ".join(line)
        print(result_line)
        return solution_rs
